...
class MetaData(models.Model):
    ...
    category = models.TextField(null=False, blank=False, max_length=10,
        choices=[(tag.value, tag.name) for tag in CategoryTypes])  
...
class DatapointV2(models.Model):
    ...
    def __str__(self):
        return "%s - %s - %s - %s "%(self.metadata.category, self.metadata.label, self.owner.username, self.starttime.strftime("%Y-%m-%d %H:%M:%S"))
